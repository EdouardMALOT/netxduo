...
NX_CRYPTO_EXTENDED_OUTPUT  extended_output;
    ...
    if(tls_session -> nx_secure_tls_1_3)
    {
         ...
         */
         ...
         /* hash_method is the signature scheme's hash: it digests the content built here and
            parameterizes the PSS encoding below. */
         ...
         /* The transcript hash was computed with the negotiated ciphersuite's hash (RFC 8446 §4.4.3),
            so its length comes from the ciphersuite — NOT from the signature scheme's hash, which may
            differ (e.g. TLS_AES_256_GCM_SHA384 suite with an rsa_pss_rsae_sha256 signature). The
